---
description: Capture lessons from this session into the studio knowledge base (runs capture-learnings).
---

Review the recent conversation and capture any worthwhile lessons into the studio
knowledge base by running the `capture-learnings` skill.

Focus on corrections the user made, repeated mistakes or dead ends, non-obvious
project facts, and patterns explicitly approved. Write one lesson per entry with
concrete `Trigger` keywords, show me what you added, and commit the change. If
nothing meets the bar, tell me so instead of inventing entries.

$ARGUMENTS
